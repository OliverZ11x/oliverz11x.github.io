...
2. 如果需要调用工具，为相应的平台生成第一批API调用（`calls`列表）。每个调用必须包含`platform`、`tool_name`和`params`。

3. **特别注意**: 如果用户的任务明确提到"下载TikTok视频并分析内容"或类似表述：

a. 如果用户提供了TikTok视频的URL，你的第一步（Stage 1 `calls`）应该是调用TikTok的`download_video`工具。例如：`{{\"platform\": \"tiktok\", \"tool_name\": \"download_video\", \"params\": {{\"url\": \"USER_PROVIDED_URL\"}}}}`。

b. 如果用户描述了一个TikTok视频但没有提供URL（例如，"下载并分析泰勒斯威夫特的最新TikTok视频"），你的第一步（Stage 1 `calls`）应该是使用TikTok的 `search_video` 或 `get_user_posts` 等工具来 *找到该视频的URL*。不要在Stage 1规划`download_video`或`analyze_local_video`。

c. **绝对不要**在Stage 1的`calls`中直接规划调用`contentunderstanding`平台的`analyze_local_video`工具。这个工具只能在Stage 2中，在视频URL被确认（且系统处理下载）之后才能被规划。

4. 生成一个初步的后续处理指令（`process_instructions`）。

5. **重要**: 如果用户请求不需要调用任何平台工具，则将 `calls` 列表设置为空 (`[]`)，并在 `direct_answer_if_no_tools` 字段中直接提供答案。
...
案例2 (TikTok下载，URL已知): {{\"calls\": [{{\"platform\": \"tiktok\", \"tool_name\": \"download_video\", \"params\": {{\"url\": \"ACTUAL_TIKTOK_VIDEO_URL_HERE\"}}}}], \"process_instructions\": \"TikTok视频URL已指定，准备下载。后续将分析内容。\", \"direct_answer_if_no_tools\": null}}

案例3 (LinkedIn工具): {{\"calls\": [{{\"platform\": \"linkedin\", \"tool_name\": \"get_profile_by_username\", \"params\": {{\"username\": \"williamhgates\"}}}}], \"process_instructions\": \"获取LinkedIn档案后...\", \"direct_answer_if_no_tools\": null}}

案例4 (无需工具): {{\"calls\": [], \"process_instructions\": \"用户问题已直接回答。\", \"direct_answer_if_no_tools\": \"太阳从东方升起。\"}}

案例5 (TikTok找视频URL，为后续下载分析做准备): {{\"calls\": [{{\"platform\": \"tiktok\", \"tool_name\": \"search_video\", \"params\": {{\"keyword\": \"taylor swift latest\"}}}}], \"process_instructions\": \"搜索泰勒斯威夫特最新TikTok视频，找到URL后准备下载和分析。\", \"direct_answer_if_no_tools\": null}}
...
A. 评估是否需进一步API调用 (Twitter、TikTok、LinkedIn) 以满足任务。

B. **关键视频分析步骤**:

- 检查 `initial_results` 中是否存在任何以 `tiktok_download_video_result_` 开头的键。

- 如果存在这样的键，其值是待分析的视频文件路径。你**必须**在 `derivative_calls` 中为此规划一个对 `contentunderstanding` 平台的 `analyze_local_video` 工具的调用。

- `analyze_local_video` 的调用格式应为: `{{\"platform\": \"contentunderstanding\", \"tool_name\": \"analyze_local_video\", \"params\": {{}} }}`. `params` 为空对象，因为系统会自动处理文件路径。

- 如果有多个待分析的视频文件路径，为每个路径都规划一个这样的调用。

C. 如果需要其他非分析类的API调用（例如，根据第一步的结果获取更多Twitter信息），将它们也加入到 `derivative_calls` 列表中。

D. 生成`final_process_instructions`指导最终整合。此指令应明确提及如果视频被分析了，则应在其最终摘要中包含视频的描述和标签。

可用工具（注意平台）：

{ALL_TOOLS_CAPABILITIES}

输出JSON示例（假设一个视频待分析，及一个额外的Twitter调用）：
...
"final_process_instructions": "已下载并分析TikTok视频。总结时请包含其内容描述和标签。同时，获取推文12345的详细信息并整合到最终答复中。"

}}

```

如果初步结果不足或失败，或者没有待分析的视频文件路径，`final_process_instructions`应指导如何向用户解释，并且 `derivative_calls` 中不应包含 `analyze_local_video`（除非有其他平台的调用需求）。
